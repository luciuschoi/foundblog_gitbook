...
이제 글을 작성해 보자. 우선 사용자 가입 후 로드인한 상태에서 홈페이지의 오른쪽에 있는 `New Post` 버튼을 클릭해 보자.
...
1번 `Category` 항목은 글을 분류하기 위한 것으로 별도의 `Category` 모델을 이용해서 데이터를 추가/삭제/변경할 수 있다. 이것은 나중에 별도로 언급할 것이다.
...
이제 루트 `URL`로 이동하면 방금 작성했던 글이 하나 보이게 된다. 이 글의 `published` 속성은 디폴트 값인 `false`를 가지고 있다.
...
우측 `sidebar` 상단에 `My Posts`라는 링크를 만들고 이 링크에 연결되는 전용 뷰 템플릿 파일을 하나 만들도록 하자.
